## Hi, I'm Alejandro

Data engineer writing about the intersection of data engineering, AI agents, MCP, and developer tools.

I publish [The Pipe & The Line](https://thepipeandtheline.substack.com) on Substack — 58+ articles and counting on building with the modern data stack and making AI actually useful for data teams.

---

### Collaborations

| Article | With |
|---|---|
| [End To End Agentic Data Modeling with OpenMetadata and MCP](https://pipeline2insights.substack.com/p/end-to-end-agentic-data-modeling-with-openmetadata-and-mcp) | [Pipeline To Insights](https://pipeline2insights.substack.com) |
| [All You Can Do Before Airflow](https://dataengineeringcentral.substack.com/p/all-you-can-do-before-airflow) | [Data Engineering Central](https://dataengineeringcentral.substack.com) |
| [How Long Until We Call AI Agents Data Products](https://moderndata101.substack.com/p/data-products-as-ai-agents) | [Modern Data 101](https://moderndata101.substack.com) |
| [Behind the Scenes of AI Observability in Production](https://www.decodingai.com/p/behind-the-scenes-of-ai-observability) | Paul / [Decoding AI](https://www.decodingai.com) |
| [Data Engineers Are Becoming MetadataOps Engineers](https://www.datagibberish.com/p/data-engineers-are-becoming-metadataops-engineers) | [Modern Data 101](https://moderndata101.substack.com) |

---

### Recent Articles

<!-- ARTICLES:START -->

| Date | Article | Tags |
|---|---|---|
| 2026-03 | [Hands-On Claude Code for Data Engineers: Data Modeling with dbt, Miro & PostgreSQL](https://thepipeandtheline.substack.com/p/claude-code-for-data-engineers-data-modeling-dbt-miro-postgresql-skills-mcp) | `ai` `dbt` |
| 2026-03 | [How Long Until We Call AI Agents Data Products](https://thepipeandtheline.substack.com/p/data-products-as-ai-agents) | `ai` `data-engineering` |
| 2026-02 | [Intro To Claude Code For Data Engineers](https://thepipeandtheline.substack.com/p/intro-claude-code-for-data-engineers) | `ai` `tools` |
| 2026-02 | [What A Time To Be A Data Engineer (On Steroids)](https://thepipeandtheline.substack.com/p/what-a-time-to-be-a-data-engineer) | `data-engineering` `ai` `career` |
| 2026-02 | [Building a Substack Agent with SKILLs and MCP](https://thepipeandtheline.substack.com/p/hands-on-building-a-substack-agent) | `ai` |
| 2026-02 | [Data Engineers Are Becoming MetadataOps Engineers](https://thepipeandtheline.substack.com/p/data-engineers-are-becoming-metadataops-engineers) | `career` `ai` `data-engineering` |
| 2026-02 | [What's the real deal about SKILLs](https://thepipeandtheline.substack.com/p/whats-the-real-deal-about-skills) | `ai` |
| 2026-02 | [Behind the Scenes of AI Observability in Production](https://thepipeandtheline.substack.com/p/behind-the-scenes-of-ai-observability) | `ai` |
| 2026-01 | [The Untold Pains About Building AI Agents On Production](https://thepipeandtheline.substack.com/p/the-untold-pains-about-building-ai) | `ai` |
| 2026-01 | [Learnings From 1 Year of Building AI Agents](https://thepipeandtheline.substack.com/p/learnings-from-1-year-of-building) | `ai` |

[See all articles →](https://thepipeandtheline.substack.com)

<!-- ARTICLES:END -->

---

### Projects

- **[substackrecapped.com](https://github.com/aboyalejandro/substackrecapped)** — Year-in-review for Substack authors. 3,000 users in the first 8 hours.
- **[substack-author-mcp](https://github.com/aboyalejandro/substack-author-mcp)** — MCP server that lets AI assistants read and analyze any Substack publication.
- **[agentic-data-modeling](https://github.com/aboyalejandro/agentic-data-modeling)** — End-to-end data modeling with OpenMetadata MCP, dbt, and Claude Code.
- **[data-stack-to-go-duckdb-dbt](https://github.com/aboyalejandro/data-stack-to-go-duckdb-dbt)** — Portable analytics stack: DuckDB + dbt + Google Sheets + Looker Studio.
- **[claude-code-data-engineers-data-modeling](https://github.com/aboyalejandro/claude-code-data-engineers-data-modeling)** — Claude Code skills and MCPs for data modeling workflows.

---

Currently building with dbt, DuckDB, Claude Code, and MCP.
